...
### Назначения системы
HseRogue — игра в жанре roguelike с консольной графикой. Игрок управляет героем: перемещает его, собирает и использует предметы, атакует врагов или избегает их. В игре есть несколько уровней. Цель игры — пройти все уровни.
...
### Контекст системы
Игра должна работать прямо на устройствах пользователей в консоли. Должны быть поддержаны Linux, Windows и MacOS
...
- Игра должна быть пошаговой. Без действий игрока ничего происходить не должно
...
#### Rationale
Компоненты logic передают GUI изменения в модели через интерфейс. Мы сделали так, чтобы GUI перерисовывали только изменившуюся часть экрана. Если перерисовывать весь экран, то он начинает мерцать
...
#### LevelGenerator
Позволяет сгенерировать уровень (игровое поле) из файла или с помощью алгоритма генерации. Уровень генерируется без героя, но возвращается его позиция
...
* `HelpState` -- перед пользователем открыта информация о настройках игры
