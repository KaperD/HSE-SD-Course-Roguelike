...
#### GameFieldBuilder
Позволяет создать игровое поле вместе с предметами и существами: случайно сгенерированное или из файла.
...
2. Затем идет строка, начинающаяся с `==`
...
4. Затем идет строка, начинающаяся с `==`
5. После этого идет список мобов (возможно пустой) в формате `x y type`
6. Затем идет строка, начинающаяся с `==`
7. После этого идет позиция героя в формате `x y`
...
#### MobFactory
Позволяет создать мобов с различным поведением
...
#### CloneMobDecorator
Моб, который каждый шаг с вероятностью cloneProbability создает свою копию в соседней клетке
...
#### MobState
Состояние моба, которое определяет стратегию моба, а также решает, когда моб должен перейти в другое состояние и в эти моменты меняет его

#### OrdinaryMobState
Стандартное состояние моба. Переводит моба в паническое состояние при уменьшении здоровья ниже healthThreshold

#### PanicMobState
Состояние моба, в котором он делает случайные шаги. Переводит моба в обычное состояние при увеличении здоровья до healthThreshold или выше
